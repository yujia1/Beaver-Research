...
class AgentService:
    ...
    async def analyze_company(self, ticker: str, company_name: str, sector: str):
        ...
        Analyze the 10-K chunks I provide for {company_name} ({ticker}) in the {sector} sector and produce a comprehensive breakdown with the following structure:
...
Today's date is {current_date}. Base your analysis strictly on the 10-K content provided. Be specific, use concrete examples from the filing, and avoid generic statements.""".format(current_date=current_date.strftime('%B %d, %Y'))
    ...
    async def analyze_operating_drivers(self, ticker: str, company_name: str, sector: str):
        ...
        """
        cached_report = self.get_cached_analysis(ticker, "operating_drivers")
        if cached_report:
            return {"report": cached_report, "cached": True}
        ...
        Based on the {sector} sector, identify the MOST CRITICAL metrics for {company_name}:
        ...
        **For each key metric**:
        ...
        - How does {company_name} compare to peers on key operating metrics?
        ...
        report_content = response.choices[0].message.content
        self.set_cached_analysis(ticker, "operating_drivers", report_content)
    ...
    async def analyze_notes_disclosures(self, ticker: str, company_name: str, sector: str):
        ...
        """
        cached_report = self.get_cached_analysis(ticker, "notes_disclosures")
        if cached_report:
            return {"report": cached_report, "cached": True}
        ...
        prompt = f"""
        Analyze the Notes & Disclosures for {company_name} ({ticker}) in the {sector} sector.
        ...
        - Revenue recognition methods typical for {sector} companies
        ...
        ### Segment Reporting
        - Expected business segments for {sector} companies
        ...
        - Common off-balance sheet arrangements in {sector}
        ...
        - Industry-specific regulatory risks for {sector}
        ...
        - **Specific risks for {company_name}**: [Identify company-specific risks]

        ### Stock-Based Compensation
        - Typical SBC as % of revenue for {sector}
        - Impact on dilution and true profitability
        - **Red flags**: Excessive SBC, frequent option repricing

        ### Customer & Supplier Concentration
        - Typical customer concentration in {sector}
        ...
        report_content = response.choices[0].message.content
        self.set_cached_analysis(ticker, "notes_disclosures", report_content)
    ...
    async def analyze_capital_structure(self, ticker: str, company_name: str, sector: str):
        ...
        """
        cached_report = self.get_cached_analysis(ticker, "capital_structure")
        if cached_report:
            return {"report": cached_report, "cached": True}
        ...
        prompt = f"""
        You are an elite Financial Analyst AI whose sole job is to produce a complete, professional financial analysis report based on the raw financial data provided for {company_name} ({ticker}) in the {sector} sector.
        ...
        **Data Handling Rules:**
        ...
        report_content = response.choices[0].message.content
        self.set_cached_analysis(ticker, "capital_structure", report_content)
