...
@router.get("/admin/reset-tables") # Changed to GET for easier browser access
def reset_tables_emergency(
    key: str
):
    if key != "emergency_secret_123":
        raise HTTPException(status_code=403, detail="Invalid key")
    """
    Emergency endpoint to reset AlphaTrade tables.
    Only accessible by authorized users (premium/admin).
    """
    from database import engine
    from models import AlphaTradePosition, AlphaTradeLot, AlphaTradeFundamentalAnalysis
    
    # Check if user is admin (optional, for now just premium is okay as this is emergency fix)
    # Ideally check current_user.role == 'admin' but verify_premium_access returns User
    ...
    try:
        ...
        results.append("Dropped FundamentalAnalysis")
    except Exception as e:
        results.append(f"Error dropping Fundamental: {e}")
        
    try:
        AlphaTradeLot.__table__.drop(engine, checkfirst=True)
        results.append("Dropped Lots")
    except Exception as e:
        results.append(f"Error dropping Lots: {e}")
        
    try:
        AlphaTradePosition.__table__.drop(engine, checkfirst=True)
        results.append("Dropped Positions")
    except Exception as e:
        results.append(f"Error dropping Positions: {e}")
    ...
    try:
        ...
        results.append("Re-created all tables")
    except Exception as e:
        results.append(f"Error re-creating tables: {e}")
        raise HTTPException(status_code=500, detail=str(e))
        
    return {"status": "Database Reset Complete", "details": results}
